...
require Rails.root.join('app/models/client/patient')
require Rails.root.join('app/models/employee/')
...
RSpec.describe Employee::Employee, type: :model do
  ...
  describe 'custom validations' do
    ...
    it 'does not allow a user associated with a client' do
      user = create(:user, :with_client)
      employee = build(:employee, user:)
      expect(employee).not_to be_valid
      expect(employee.errors[:user]).to include('is already associated with a client')
    end
  ...
  # Deletion behavior
  describe 'deletion behavior' do
    it 'deletes associated user when employee is deleted' do
      employee = create(:employee)
      user = employee.user
      expect { employee.destroy }.to change { User.count }.by(-1)
